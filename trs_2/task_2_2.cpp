...
int main()
{
	...
	for (int k = 0; k < m.size(); k++)
	{
		...
		vector<double> C(m[k]);
		vector<double> F(m[k]);
		vector<double> U_next(m[k]);

		for (int i = 1; i < m[k] - 1; i++)
		{
			A[i] = -1. / (8 * pi * pi * h * h);
			B[i] = 1. / tau + 1. / (4 * pi * pi * h * h);
			C[i] = -1. / (8 * pi * pi * h * h);
		}
		...
		for (int j = 1; j < n[k]; j++)
		{
			...
			{
				double x = i * h;
				F[i] = u[j - 1][i] / tau + sin(2 * pi * x) - 1. / 2. / pi / pi + u[j - 1][i - 1] / (8 * pi * pi * h * h) -
					u[j - 1][i] / (4 * pi * pi * h * h) + u[j - 1][i + 1] / (8 * pi * pi * h * h);
			}
			F[m[k] - 1] = 3 + 2 * pi * (1 - exp(-t));

			//vector<double> U_next;
			tridiagonal_matrix(m[k], A, B, C, F, u[j]);
